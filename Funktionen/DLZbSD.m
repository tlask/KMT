function [HolArtOrder,AufKB,A,C,Auftrag_fertig] = DLZbSD(A,HZ,KZ,SP,ArtOrder,AufOrder,AufKB,HolArtOrder,C,Auftrag_fertig,Artikel_neu_berechnen)
%
% [Z,HolArtOrder] = <strong>DLZbS</strong> (A,HZ,KZ,SP,ArtOrder,AufOrder,AufKB,HolArtOrder,C,Auftrag_fertig,p,Artikel_neu_berechnen)
%
        ...
        if sum(AKB(q,:)) > 0
            for j = AufKB(1:SP)
                if sum(AKB(:,find(AufKB==j))) > 0 %#ok<FNDSB>
                   C(1,j) =  C(1,j) + HZart(q) + KZart(q)*sum(AKB(q,:)); 
                end
            end
