function [Z,HolArtOrder,e] = UdFbSR(A,HZ,KZ,SP,DL,ArtOrder,AufOrder)
%
% [Z,HolArtOrder,e] = <strong>UdFbSR</strong>(A,HZ,KZ,SP,ArtOrder,AufOrder)
%
% Zielfunktion mit Stellplatzbegrenzung:
%   Diese Zielfunktion ist zur Minimierung der summierten Überschreitungen
%   der Fertigstellungszeitpunkte der Aufträge, wobei die Anzahl der
%   Stellplätze begrenzt ist und ein rollierendes Wiederholprinzip für
%   die Artikel verwendet wird.
%
% Parameter:
...
Z = 0; % summierte Überschreitung der Deadlines
for i = 1:n
    if e(i) > 0       % Für jeden Auftrag, dessen Überschreitung des     
        Z = Z + e(i); % Fertigstellungszeitpunkts e(i) > 0 ist wird der
    end               % Zielfunktionswert Z um e(i) erhöht. 
end 
