function [Order] = minAminH(A,HA)
%
% [Order] = <strong>minAminH</strong>(A,HA)
%
% Artikelreihenfolgeverfahren:
%   Anordnung nach minimaler Auftragslänge und minimaler Häufigkeit
%
% Parameter:
...
for i = 1:m
    ...
    if tmp_exist
        HAminArt = find(HA(BenoetigteArt)==min(HA(BenoetigteArt)),1,'first');
        Order(i) = BenoetigteArt(HAminArt);
        A(BenoetigteArt(HAminArt),:) = zeros(1,n);
        AL = AuftragsLaenge(A);
    else
